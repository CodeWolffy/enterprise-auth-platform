...
代码生成使用内置默认模板，不再支持数据库自定义模板。

自动注册：当 `autoRegister=true` 时，生成完成后会在 `sys_menu` 写入：

- 父目录 `/platform/generated`（仅一次）
- `/<platform>/generated/<module>` 生成模块菜单
- `<module>:page/get/add/edit/del` 按钮权限
- 同步把以上菜单权限（含祖先）授权给当前租户和 ADMIN 角色，并清空权限缓存
