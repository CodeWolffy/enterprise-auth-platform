...
- `POST /api/workflow/process-definitions`：创建草稿
- `GET /api/workflow/process-definitions`：分页查询
- `GET /api/workflow/process-definitions/{definitionId}`：详情
- `PUT /api/workflow/process-definitions/{definitionId}/deploy`：部署
- `PUT /api/workflow/process-definitions/{definitionId}/disable`：停用
- `POST /api/workflow/instances`：发起
- `GET /api/workflow/instances/my`：我的发起
- `GET /api/workflow/instances/{instanceId}`：实例详情
- `PUT /api/workflow/instances/{instanceId}/withdraw`：撤回（仅发起人）
- `PUT /api/workflow/instances/{instanceId}/terminate`：终止（需 `workflow:write`）
- `GET /api/workflow/tasks/todo`：我的待办
- `GET /api/workflow/tasks/done`：我的已办
- `PUT /api/workflow/tasks/{taskId}/approve`：审批通过
- `PUT /api/workflow/tasks/{taskId}/reject`：驳回（按节点的 `rejectStrategy` 决定是结束、回退上一步还是回到起点）
- `PUT /api/workflow/tasks/{taskId}/transfer`：转签
- `PUT /api/workflow/tasks/{taskId}/urge`：催办
- `GET /api/workflow/tasks/{taskId}/urges`：任务催办历史
- `GET /api/workflow/instances/{instanceId}/urges`：流程实例催办分页

流程步骤结构（含 `rejectStrategy`）：
...
- `END`（默认）：驳回即结束流程，实例状态置为 `REJECTED`。
- `PREVIOUS`：驳回到上一节点，首节点驳回时退化为 `END`。
- `RESTART`：驳回后重新从第一个节点开始。

### 代码生成

- `GET /api/codegen/tables`：分页查询可生成数据表
- `GET /api/codegen/tables/{tableName}`：表字段
- `POST /api/codegen/preview`：预览生成结果（受 `selectedFiles`、`autoRegister` 控制）
- `POST /api/codegen/generate`：写入隔离目录（默认 `target/generated-codegen`），并按 `selectedFiles` 选择性落盘
- `POST /api/codegen/download`：把所选文件打包为 ZIP 直接下载（不落盘）
- `GET /api/codegen/templates`：分页查询自定义模板
- `GET /api/codegen/templates/{templateId}`：模板详情
- `POST /api/codegen/templates`：新增模板
- `PUT /api/codegen/templates/{templateId}`：修改模板
- `DELETE /api/codegen/templates/{templateId}`：删除模板（内置不可改删）
...
```json
{
  ...
  "language": "java",
  "pathPattern": "Controller.java",
  "content": "/* Generated by {{moduleName}} at {{generatedAt}} */\n…",
  "description": "生成器头部信息"
}
```

模板支持占位符（按文件渲染时替换）：`{{tableName}} {{moduleName}} {{className}} {{lowerClassName}} {{kebabName}} {{title}} {{packageName}} {{primaryKeyColumn}} {{primaryKeyField}} {{primaryKeyJavaType}} {{generatedAt}}`。生成时按 `language + pathPattern`（支持关键字或正则）匹配首个命中模板，未命中则使用内置默认实现。

自动注册：当 `autoRegister=true` 时，生成完成后会在 `sys_resource` 写入：

- 父目录 `generated`（仅一次）
- `generated.<module>` 菜单资源（`grantKey=<module>:read`）
- `api.generated.<module>.read` / `.write` API 资源
- 同步把以上资源（含祖先）授权给平台 ADMIN 角色，并清空权限缓存
