...
const assignedMenus = computed(() => {
  const menuMap = new Map(
    flattenMenus(detailMenuTree.value).map((menu: any) => [String(menu.id), menu]),
  );
  const ids = (detailData.value?.menuIds as Array<number | string> | undefined) ?? [];
...
const detailPackageName = computed(() => {
  ...
  const pkg = pkgList.value.find((p: any) => p.packageCode === detailData.value.packageCode);
...
const upMenu = (id: string) => {
  ...
  void invokeWhenComponentReady(tenantMenuRef, (menu: any) => menu.initMenu(id));
};
</script>
...
<template>
  <div class="hx-layout-container">
    <div class="hx-layout-container-auto hx-layout-container-view">
      ...
      <TenantMenu v-if="menuMounted" ref="tenantMenuRef" @init-page="initPage" />
      ...
      <ElTable v-loading="loading" :data="state.tableData" border>
        ...
        <ElTableColumn label="联系人" min-width="150">
          <template #default="scope">
            <div style="display: grid; gap: 2px">
              ...
              <span style="font-size: 12px; color: #909399">{{ scope.row.contactPhone || '-' }}</span>
            </div>
          </template>
        </ElTableColumn>
        ...
        <ElTableColumn prop="tenantStatus" label="状态" width="90">
          <template #default="scope">
            <DictTag :options="status" :value="String(scope.row.tenantStatus)" />
          </template>
        </ElTableColumn>
    ...
    <ElDrawer v-model="detailVisible" title="租户详情" size="600px">
      <div v-loading="detailLoading">
        <ElDescriptions :column="1" border v-if="detailData">
          <ElDescriptionsItem label="租户编码">{{ detailData.tenantId }}</ElDescriptionsItem>
          <ElDescriptionsItem label="租户名称">{{ detailData.name }}</ElDescriptionsItem>
          <ElDescriptionsItem label="租户级别">
            <ElTag :type="isPlatformTenant(detailData) ? 'danger' : 'info'" size="small">
              {{ isPlatformTenant(detailData) ? '平台级' : '业务级' }}
            </ElTag>
          </ElDescriptionsItem>
          <ElDescriptionsItem label="套餐">{{ detailPackageName }}</ElDescriptionsItem>
          <ElDescriptionsItem label="官网">{{ detailData.website || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="地址">{{ detailData.address || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="联系人">{{ detailData.contactName || '-' }}</ElDescriptionsItem>
          ...
          <ElDescriptionsItem label="授权开始">{{ formatDateTime(detailData.authBeginAt) }}</ElDescriptionsItem>
          <ElDescriptionsItem label="授权到期">{{ formatDateTime(detailData.expireAt) }}</ElDescriptionsItem>
          <ElDescriptionsItem label="状态">
            <DictTag :options="status" :value="String(detailData.tenantStatus)" />
          </ElDescriptionsItem>
          <ElDescriptionsItem label="Logo">{{ detailData.logoUrl || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="创建时间">{{ formatDateTime(detailData.createTime) }}</ElDescriptionsItem>
        </ElDescriptions>

        <div v-if="assignedMenus.length" style="margin-top: 20px">
          <h4 style="margin-bottom: 10px">已分配菜单 ({{ assignedMenus.length }} 项)</h4>
    ...
    <ElDrawer v-model="historyVisible" title="租户变更历史" size="800px">
      <!-- 汇总 -->
      <div v-if="historySummary" style="margin-bottom: 16px; display: flex; gap: 12px">
        <ElCard shadow="never" style="flex: 1">
          <ElStatistic title="变更总数" :value="historySummary.totalChanges ?? 0" />
        </ElCard>
        <ElCard shadow="never" style="flex: 1">
          <ElStatistic title="创建事件" :value="historySummary.createCount ?? 0" />
        </ElCard>
        <ElCard shadow="never" style="flex: 1">
          <ElStatistic title="更新事件" :value="historySummary.updateCount ?? 0" />
        </ElCard>
      </div>
      ...
      <ElForm :inline="true" style="margin-bottom: 12px">
        <ElFormItem label="变更类型">
          <ElSelect v-model="historyQuery.changeType" clearable placeholder="全部" style="width: 120px">
        ...
        <ElFormItem label="字段">
          <ElInput v-model="historyQuery.fieldKey" clearable placeholder="字段名" style="width: 140px" />
        </ElFormItem>
        <ElFormItem label="操作人">
          <ElInput v-model="historyQuery.operator" clearable placeholder="操作人" style="width: 140px" />
        </ElFormItem>
        <ElFormItem>
          <ElButton type="primary" @click="historyQuery.current = 1; loadHistory()">搜索</ElButton>
      ...
      <ElTable v-loading="historyLoading" :data="historyData" border stripe>
        <ElTableColumn prop="changeType" label="类型" width="90">
          <template #default="{ row }">
            <ElTag size="small" :type="row.changeType === 'CREATE' ? 'success' : row.changeType === 'DELETE' ? 'danger' : 'info'">
              {{ row.changeType }}
            </ElTag>
          </template>
        </ElTableColumn>
        <ElTableColumn prop="fieldKey" label="字段" width="140" show-overflow-tooltip />
        <ElTableColumn prop="oldValue" label="旧值" min-width="120" show-overflow-tooltip />
        <ElTableColumn prop="newValue" label="新值" min-width="120" show-overflow-tooltip />
        <ElTableColumn prop="operator" label="操作人" width="120" />
        <ElTableColumn label="时间" width="170">
          <template #default="{ row }">{{ formatHistoryTime(row.occurredAt) }}</template>
        </ElTableColumn>
        <template #empty>
