  ...
  roles.value.filter((role) => {
    const matchesKeyword =
      ...
      [role.code, role.name, role.description || ''].some((value) =>
        value.toLowerCase().includes(keyword.value.trim().toLowerCase()),
      )
...
const allScopeCount = computed(() => roles.value.filter((item) => item.dataScopeType === 'ALL').length)
const deptScopeCount = computed(() => roles.value.filter((item) => item.dataScopeType === 'DEPT_AND_CHILDREN').length)
const customScopeCount = computed(() => roles.value.filter((item) => item.dataScopeType === 'CUSTOM').length)
