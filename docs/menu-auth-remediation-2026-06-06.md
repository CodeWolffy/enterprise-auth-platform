...
本次验收发现，当前菜单管理改造方向基本正确：已经从旧的 `sys_resource` 兼做菜单，推进到标准化的 `sys_menu + sys_role_menu` 模型。但现状仍存在权限来源未收敛、菜单种子数据不一致、编辑父级丢失、能力菜单未进入运行链路等问题。
