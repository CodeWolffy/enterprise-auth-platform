...
| `ResourceQueryFacade` | 菜单、按钮、资源树只读查询契约 | ✅ 已完成第一步：菜单树和权限码查询已通过 resource facade 暴露 |
| `RoleGrantQueryFacade` | 角色授权、权限码只读查询契约 | ⏳ 后续随 role 拆分补齐 |
| `TenantResourcePolicyFacade` | 租户资源覆盖、可见性只读查询契约 | ⏳ 后续随 tenant/resource 拆分补齐 |
...
| 目标服务 | 职责 | 缓存要求 | 状态 |
|---|---|---|---|
| `DictApplicationService` | 字典管理 | 明确字典查询缓存 key 与写后失效范围 | ✅ 已完成第一步：控制器已切入应用层入口，旧服务暂作兼容底座 |
| `ConfigApplicationService` | 系统参数管理 | 明确参数缓存是否租户隔离、是否允许运行期刷新 | ✅ 已完成第一步：控制器已切入应用层入口，旧服务暂作兼容底座 |
| `NoticeApplicationService` | 公告管理 | 明确公告是否缓存、按租户还是全局读取 | ✅ 已完成第一步：控制器已切入应用层入口，旧服务暂作兼容底座 |
| `CategoryRuleApplicationService` | 分类规则管理 | 明确分类规则被哪些页面/接口复用，变更后如何失效 | ✅ 已完成第一步：控制器已切入应用层入口，旧服务暂作兼容底座 |
...
| 顺序 | 任务 | 状态目标 | 状态 |
|---|---|---|---|
| 1 | 锁定后端主链路测试基线 | 登录、`/auth/me`、会话、租户、权限、审计、多租户链路可回归 | ✅ 已完成：`mvn "-Dmaven.repo.local=.m2repo" test` 通过，102 个用例 0 失败 |
...
| 7 | 收敛 `PermissionSnapshotService` 契约 | `/auth/me` 菜单、权限码、routeKey、grantKey 稳定 | ✅ 已完成：`/api/auth/me` 已由 `PermissionSnapshotApplicationService` 输出统一快照，新增菜单、权限码、`routeKey`、`path`、`component` 契约回归 |
| 8 | 建立前端 `app/registry/` 与模块 manifest | 路由、菜单、权限、图标、标题先统一声明，暂不强制动态注册 | ✅ 已完成：新增 `frontend/src/app/registry/module-manifest.ts`，路由定义与 `routeKey` 访问映射已统一从 manifest 派生；`npm run build` 通过 |
| 9 | 拆分租户域 | 租户生命周期、租户目录、变更日志、访问策略职责清晰 | ⏳ 进行中：控制器已切入 tenant application service；租户目录查询已收敛到 `TenantDirectoryApplicationService`；变更日志查询/摘要/写入已集中；访问策略已抽出；租户能力覆盖入口已从生命周期服务收敛到 `TenantCapabilityApplicationService`；旧 `TenantManagementService` 已移除历史查询/摘要转发，避免变更日志职责回流；租户控制器回归 6 个用例通过 |
| 10 | 拆分系统域 | 字典、参数、公告、分类规则独立，并明确缓存失效口径 | ✅ 已完成第一步：控制器已按字典、参数、公告、分类规则切入 application service；旧 `SystemManagementService` 暂作兼容底座 |
