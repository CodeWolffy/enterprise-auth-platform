...
| 4B.3 | 拆分认证状态 | ⏳ 后续推进：认证态、会话态、租户态、权限快照职责继续从 `auth` store 拆分 |
| 4B.4 | 增强 HTTP 请求治理 | ⏳ 后续推进：幂等请求可重试，写操作默认不重试，支持请求取消和统一错误码 |
| 4B.5 | 抽出通用 CRUD 组合能力 | ⏳ 后续推进：列表、搜索、表单弹窗、详情抽屉不再每个页面重复写 |
