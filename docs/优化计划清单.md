...
| `system` | 字典、参数、公告、分类规则 | `DictApplicationService`、`ConfigApplicationService`、`NoticeApplicationService`、`CategoryRuleApplicationService` |
...
| 子域 | 目标服务 | 职责 | 状态 |
|---|---|---|---|
| tenant lifecycle | `TenantLifecycleApplicationService` | 租户创建、更新、删除、状态治理、资源覆盖入口 | ✅ 已完成：租户创建、更新、删除已由生命周期应用服务承接，控制器不直接依赖旧管理服务 |
| tenant catalog | `TenantCatalogApplicationService`、`TenantCapabilityApplicationService` | 套餐、能力、套餐能力关系、影响分析 | ✅ 已完成：套餐与能力目录由独立应用服务承接，租户能力覆盖对外视图已归入租户模块应用层 |
| tenant change log | `TenantChangeLogApplicationService` | 租户变更日志、历史摘要、影响说明 | ✅ 已完成：历史查询、摘要、写入、影响说明与对外视图模型已集中到独立入口 |
| tenant access | `TenantAccessPolicy` | 租户可见性、平台管理员判断 | ✅ 已完成：可见性与平台超级管理员判断已从旧服务抽成独立策略，并被目录、变更日志和能力覆盖链路复用 |

验收标准：

- 租户列表、创建、更新、删除、能力覆盖、历史查询行为不变。
- `TenantManagementService`、`TenantCatalogManagementService`、`CatalogService` 的职责重新分配清晰；Controller 对外只暴露租户模块应用层视图模型。
- 变更日志逻辑集中，不散落在多个服务内。
- 套餐能力逻辑可被后续新项目复用。

当前验证：

- ✅ 租户控制器已切断对旧 `TenantManagementService` 嵌套视图模型的直接暴露：历史、历史摘要、能力覆盖响应类型均归入租户模块应用层。
- ✅ `mvn "-Dmaven.repo.local=.m2repo" test` 通过：102 tests, 0 failures, 0 errors。
...
| 序号 | 事项 | 范围 | 验收标准 | 状态 |
|---|---|---|---|---|
| 3.1 | 引入 Flyway | `pom.xml`、`src/main/resources/db/migration` | 新环境启动时可自动执行迁移 | ✅ 已完成：Flyway 已成为唯一数据库初始化入口，迁移目录与依赖已固定 |
| 3.2 | 当前 SQL 转基线迁移 | `enterprise_auth_platform.sql` → `V1__baseline.sql` | 当前结构和基础数据能作为 V1 初始化 | ✅ 已完成：旧完整 SQL 已复制为 `V1__baseline.sql`，并作为基线快照保留 |
| 3.3 | 建立迁移命名规范 | `Vxxx__description.sql` | 后续结构、索引、种子数据都走版本化脚本 | ✅ 已完成：已固定为 `V{yyyyMMddHHmm}__{module}_{description}.sql`，后续只追加不回改 |
| 3.4 | 核验索引 | 用户、审计、租户、角色资源表 | 只根据实际查询路径补充索引，不盲目照旧计划添加 | ✅ 已完成：用户、角色资源、租户覆盖主路径索引已覆盖，审计查询组合索引已补齐 |
| 3.5 | 明确全局表与租户表 | MyBatis 租户拦截配置 | 全局表不被错误追加租户条件 | ✅ 已完成：`sys_tenant` 保持忽略，套餐/能力目录通过显式 `platform` 租户承载 |
| 3.6 | 同步 CI 初始化路径 | `.github/workflows/ci.yml` | CI 不再依赖手工导入旧 SQL，改为 Flyway 初始化或测试启动迁移 | ✅ 已完成：CI 仅保留 Flyway 迁移路径，测试启动即执行校验 |
| 3.7 | 同步部署与启动文档 | `README.md`、`DEPLOYMENT_CHECKLIST.md` | 删除“手工执行完整 SQL”作为主路径，改为 Flyway 主路径 | ✅ 已完成：README 与部署检查清单已统一为 Flyway 主路径，并保留 baseline 说明 |
...
| 序号 | 事项 | 验收标准 | 状态 |
|---|---|---|---|
| 4A.1 | 建立 `ModuleRegistry` | routeKey、path、component、title、icon、permission 从模块清单汇总 | ✅ 已完成：模块 manifest 已成为路由、菜单、权限声明的统一来源 |
| 4A.2 | 迁移现有路由定义 | `router/index.ts` 不再单独维护完整业务路由映射 | ✅ 已完成：业务路由已由 manifest 汇总派生，路由入口只负责装配与守卫 |
| 4A.3 | 迁移菜单图标和标题映射 | `AppNav.vue` 不再维护独立 `iconMap` / `titleMap` | ✅ 已完成：导航标题、图标、路径与权限入口已收敛到模块声明 |
| 4A.4 | 收敛 routeKey/path 权限映射 | `route-access.ts` 消费统一 manifest，而不是另写一套映射 | ✅ 已完成：访问映射从 manifest 派生，避免 routeKey、path、permission 多处漂移 |
| 4A.5 | 保留静态路由注册 fallback | 权限、刷新、直达 URL 行为不变 | ✅ 已完成：动态注册前后保留登录页、404 与布局兜底链路 |

#### 阶段 4B：真正动态路由与状态拆分

| 序号 | 事项 | 验收标准 | 状态 |
|---|---|---|---|
| 4B.1 | 实现真正动态路由注册 | 动态路由按权限快照注入，重复注册前清理旧动态路由 | ✅ 已完成：`registerDynamicRoutes` 已按权限快照注入可访问业务路由，并支持重复注册前清理旧动态路由 |
| 4B.2 | 处理刷新与直达 URL 边界 | 刷新恢复后可重新注册路由，深层地址可回到原目标页 | ✅ 已完成：会话恢复后按权限快照重新注册路由，深层地址首跳命中兜底时会触发原地址重匹配 |
| 4B.3 | 拆分认证状态 | 认证态、租户态、权限快照职责分离，旧调用方保持兼容 | ✅ 已完成：`auth` store 保持兼容门面，认证态、租户态、权限快照已拆到独立 store |
| 4B.4 | 增强 HTTP 请求治理 | 幂等请求、写操作、取消作用域、错误解析规则明确 | ✅ 已完成：幂等请求默认可重试，写操作默认不重试，支持请求取消作用域和统一错误信息解析 |
| 4B.5 | 抽出通用 CRUD 组合能力 | 列表、搜索、重置、分页、刷新、删除后回载形成通用状态模型 | ✅ 已完成：新增通用列表组合能力，覆盖列表、搜索、重置、分页、刷新、删除后回载状态模型 |
...
状态：✅ 已完成第二轮：缓存名称已统一到 `common/cache` 登记入口；认证主体、注册策略、系统字典、系统参数、系统公告、系统分类缓存均具备独立 TTL 配置；部署检查清单已补充缓存命名空间和 TTL 核验项。
...
状态：✅ 已完成第二轮：后端新增 `PermissionCodes` 权限登记入口，Controller 权限注解已切换为常量引用；前端新增 `permissions.ts` 权限声明入口，模块 manifest 已改为引用统一权限常量。
...
状态：✅ 已完成：用户列表已从“全量查询后内存过滤”改为数据范围 SQL 条件下推，并使用数据库分页；补充了分页前应用数据范围的回归用例。
...
状态：✅ 已完成：新增审计事件发布契约 `common/audit/AuditEventPublisher`，当前 `AuditService` 继续同步落库，但业务侧已具备面向事件发布的统一入口。
...
状态：✅ 已完成：补齐 `dev`、`staging`、`prod` 配置边界，新增本地配置示例模板，并同步 README 与部署检查清单。
...
状态：✅ 已完成：前端新增 Vitest 单元测试入口，覆盖权限快照 store 与通用 CRUD 组合能力；CI 已加入 `npm run test:unit`，继续保留后端 verify、前端 lint/build 与 E2E；本轮已复验 `npm run test:unit`、`npm run lint`、`npm run build` 与 `mvn "-Dmaven.repo.local=.m2repo" "-DskipTests" compile` 均通过。
...
状态：✅ 已完成：README、部署检查清单、认证会话文档、基座扩展指南、模块边界规范、新项目初始化清单已形成闭环；本轮同步补齐 Vitest 单测门禁与 CI 质量门禁说明。
...
| 9 | 拆分租户域 | 租户生命周期、租户目录、变更日志、访问策略职责清晰 | ✅ 已完成：控制器已切入 tenant application service；租户目录查询已收敛到 `TenantDirectoryApplicationService`；套餐与能力目录已分别由 `TenantCatalogApplicationService`、`TenantCapabilityApplicationService` 承接；变更日志查询/摘要/写入与响应模型已集中；访问策略已抽出；租户能力覆盖对外视图已归入租户模块应用层；`TenantControllerTest` + `TenantCatalogControllerTest` 共 6 个用例通过 |
...
| 11 | 引入 Flyway 基线迁移 | DB 初始化进入版本化迁移，CI / README / 部署清单同步更新 | ✅ 已完成：Flyway 依赖、V1 基线、审计索引增量、CI、README、部署清单已同步；本地历史库 baseline 路径已验证 |
