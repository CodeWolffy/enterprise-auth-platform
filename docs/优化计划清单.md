...
| `ResourceQueryFacade` | 菜单、按钮、资源树只读查询契约 |
| `RoleGrantQueryFacade` | 角色授权、权限码只读查询契约 |
| `TenantResourcePolicyFacade` | 租户资源覆盖、可见性只读查询契约 |
...
| 子域 | 目标服务 | 职责 |
|---|---|---|
| tenant lifecycle | `TenantLifecycleApplicationService` | 租户创建、更新、删除、状态治理、资源覆盖入口 |
| tenant catalog | `TenantCatalogApplicationService`、`TenantCapabilityApplicationService` | 套餐、能力、套餐能力关系、影响分析 |
| tenant change log | `TenantChangeLogApplicationService` | 租户变更日志、历史摘要、影响说明 |
| tenant access | `TenantAccessPolicy` | 租户可见性、平台管理员判断 |
...
| 目标服务 | 职责 | 缓存要求 |
|---|---|---|
| `DictApplicationService` | 字典管理 | 明确字典查询缓存 key 与写后失效范围 |
| `ConfigApplicationService` | 系统参数管理 | 明确参数缓存是否租户隔离、是否允许运行期刷新 |
| `NoticeApplicationService` | 公告管理 | 明确公告是否缓存、按租户还是全局读取 |
| `CategoryRuleApplicationService` | 分类规则管理 | 明确分类规则被哪些页面/接口复用，变更后如何失效 |
...
| 序号 | 事项 | 范围 | 验收标准 |
|---|---|---|---|
| 3.1 | 引入 Flyway | `pom.xml`、`src/main/resources/db/migration` | 新环境启动时可自动执行迁移 |
| 3.2 | 当前 SQL 转基线迁移 | `enterprise_auth_platform.sql` → `V1__baseline.sql` | 当前结构和基础数据能作为 V1 初始化 |
...
| 3.6 | 同步 CI 初始化路径 | `.github/workflows/ci.yml` | CI 不再依赖手工导入旧 SQL，改为 Flyway 初始化或测试启动迁移 |
| 3.7 | 同步部署与启动文档 | `README.md`、`DEPLOYMENT_CHECKLIST.md` | 删除“手工执行完整 SQL”作为主路径，改为 Flyway 主路径 |
...
| 序号 | 事项 | 验收标准 |
|---|---|---|
| 4B.1 | 实现真正动态路由注册 | `registerDynamicRoutes` 不再是空操作，并能根据权限快照注册可访问路由 |
| 4B.2 | 保留旧路由 fallback 一个迭代 | 动态注册异常时不影响核心页面访问 |
| 4B.3 | 拆分认证状态 | 认证态、会话态、租户态、权限快照职责分离 |
| 4B.4 | 增强 HTTP 请求治理 | 幂等请求可重试，写操作默认不重试，支持请求取消和统一错误码 |
| 4B.5 | 抽出通用 CRUD 组合能力 | 列表、搜索、表单弹窗、详情抽屉不再每个页面重复写 |
...
| 顺序 | 任务 | 状态目标 | 状态 |
|---|---|---|---|
| 1 | 锁定后端主链路测试基线 | 登录、`/auth/me`、会话、租户、权限、审计、多租户链路可回归 | ⏳ 待执行 |
| 2 | 锁定前端 E2E 基线 | 登录、菜单、权限守卫、关键 CRUD 能稳定跑通 | ⏳ 待执行 |
| 3 | 建立后端八模块空骨架（含 `dept`） | 新旧代码可并存迁移，不搬大业务代码 | ✅ 已完成 |
| 4 | 增加最小模块边界校验 | 禁止跨模块引用对方 `domain` / `infrastructure` | ✅ 已完成 |
| 5 | 迁移低风险横切能力 | `common/context`、`common/web`、`common/authz`、`infrastructure/security` 等先归位 | ⏳ 待执行 |
| 6 | 拆分 `AuthService` | 登录、注册、会话、租户切换独立，接口行为不变 | ⏳ 待执行 |
| 7 | 收敛 `PermissionSnapshotService` 契约 | `/auth/me` 菜单、权限码、routeKey、grantKey 稳定 | ⏳ 待执行 |
| 8 | 建立前端 `app/registry/` 与模块 manifest | 路由、菜单、权限、图标、标题先统一声明，暂不强制动态注册 | ⏳ 待执行 |
| 9 | 拆分租户域 | 租户生命周期、租户目录、变更日志、访问策略职责清晰 | ⏳ 待执行 |
| 10 | 拆分系统域 | 字典、参数、公告、分类规则独立，并明确缓存失效口径 | ⏳ 待执行 |
| 11 | 引入 Flyway 基线迁移 | DB 初始化进入版本化迁移，CI / README / 部署清单同步更新 | ⏳ 待执行 |
| 12 | 补齐扩展文档 | 新模块、新项目有明确接入方式 | ⏳ 待执行 |

第一轮不做：

- 不全量迁移所有 Entity / Mapper / DTO。
- 不立即删除所有旧包。
- 不直接启用完整动态路由，先完成 manifest 收敛。
