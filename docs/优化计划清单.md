...
| 4B.3 | 拆分认证状态 | 认证态、租户态、权限快照职责分离，旧调用方保持兼容 | ✅ 已完成第一轮：`auth` store 保持兼容门面，认证态、租户态、权限快照已拆到独立 store |
| 4B.4 | 增强 HTTP 请求治理 | 幂等请求、写操作、取消作用域、错误解析规则明确 | ✅ 已完成第一轮：幂等请求默认可重试，写操作默认不重试，支持请求取消作用域和统一错误信息解析 |
| 4B.5 | 抽出通用 CRUD 组合能力 | 列表、搜索、重置、分页、刷新、删除后回载形成通用状态模型 | ✅ 已完成第一轮：新增通用列表组合能力，覆盖列表、搜索、重置、分页、刷新、删除后回载状态模型 |
