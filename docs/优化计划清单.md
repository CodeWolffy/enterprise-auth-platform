...
> 更新时间：2026-05-26（v2 修订：模块边界与目录口径收敛）  
> 项目状态：未上线，可进行结构性重构  
...
│   ├── security/             # 真正跨模块共享：DataScopeService、PlatformAdminSupport、权限码常量
...
| 文件 | 归属 |
|---|---|
| `AuthContextHolder`、`SecuritySupport`、`PlatformAdminSupport`、`DataScopeService` | `common/security/`（真跨模块共享） |
...
目标：在大改前锁定现状、修正过期计划、建立可回归基线。

执行项：

| 序号 | 事项 | 范围 | 验收标准 |
|---|---|---|---|
| 0.1 | 重写优化计划清单 | `docs/优化计划清单.md` | 文档不再包含过期 Vite P0、测试不存在等错误判断 |
| 0.2 | 盘点现有测试并标记基线 | `src/test/java/.../platform/` 已分包：`auth`、`audit`、`dept`、`resource`、`system`、`tenant`、`user` | 列出每包覆盖的链路，**缺口处补关键集成测试**，而非另起一套 |
| 0.3 | 锁定回归链路清单 | 登录、登出、会话恢复、`/auth/me` 权限快照、租户切换、强制下线、用户/角色/资源 CRUD、审计写入与导出 | 每条链路至少 1 个可执行测试 |
| 0.4 | 明确第一轮改造边界 | 后端、前端、数据库 | 第一轮只做基座骨架和关键拆分，不一次性重写所有业务 |
| 0.5 | 每阶段独立 PR + 标签 | Git | 形成 `phase-0`、`phase-1` 等标签，便于回滚 |
...
目标：建立 `common / modules / infrastructure` 三类边界，为后续大 Service 拆分提供落点。

执行项：

| 序号 | 事项 | 范围 | 验收标准 |
|---|---|---|---|
| 1.1 | 建立目标目录骨架 | `src/main/java/.../platform` | 目录边界清晰，无业务代码混入技术设施目录 |
| 1.2 | 扁平化 `common/convention` | 当前 `common/convention/{annotation,exception,filter,result,web}` | 移除 `convention` 中间层，直接 `common/web`、`common/exception` |
| 1.3 | 迁移共享上下文与 Web 约定 | `common/context`、`common/web` | `TenantContext`、`RequestContext`、`ApiResponse`、`GlobalExceptionHandler` 归位 |
| 1.4 | 拆解现有 `security/` 包 | 按 3.1 节三分法 | `common/security`、`modules/auth/`、`infrastructure/security/` 各自归位 |
| 1.5 | 迁移技术配置与业务 Properties | `infrastructure/config`、各 `modules/*/infrastructure/` | 纯技术配置入 infrastructure；`RegistrationProperties`、`TenantProperties` 等跟随模块 |
| 1.6 | 建立八个业务模块空壳 | `modules/{auth,user,role,resource,dept,tenant,system,audit}` | 每个模块至少有 `interfaces/application/` 两层，domain/infrastructure 可按需添加 |
| 1.7 | 保持 API 路径兼容 | Controller 层 | 包路径变化不影响 `/api/**` URL，前端无感切换 |
| 1.8 | （可选）引入 ArchUnit 守护规则 | `src/test/java/.../arch/` | 自动校验"模块不互相引用 domain/infrastructure"等约束 |
...
优先级：P0  
建议周期：**2-3 周（Auth → Tenant → System 串行，每个子项 3-5 天 + 回归验证 1-2 天）**

目标：拆掉当前最影响扩展性的核心大 Service。

**串行顺序原因**：

- `AuthService` 是入口链路，先拆能尽早暴露契约问题。
- `TenantManagementService` 与租户切换、能力覆盖耦合，紧跟 Auth。
- `SystemManagementService` 是子域明确的字典/参数/公告/分类，可作为"标准模板"巩固模式。
...
| 目标服务 | 职责 |
|---|---|
| `LoginApplicationService` | 登录认证、验证码校验、登录风控、Sa-Token 登录 |
| `RegistrationApplicationService` | 注册策略、注册防刷、注册审计 |
| `SessionApplicationService` | 登出、会话列表、强制下线、会话索引 |
| `TenantSwitchApplicationService` | 平台管理员租户切换、权限快照刷新 |
| `PermissionSnapshotApplicationService` | 当前用户权限快照编排 |
...
#### 2.2 `TenantManagementService` 拆分

目标拆分：

| 目标服务 | 职责 |
|---|---|
| `TenantLifecycleApplicationService` | 租户创建、更新、删除、状态治理 |
| `TenantCapabilityApplicationService` | 套餐、能力、覆盖配置 |
| `TenantChangeLogApplicationService` | 租户变更日志、历史摘要、影响说明 |
| `TenantAccessPolicy` | 租户可见性、平台管理员判断 |
...
#### 2.3 `SystemManagementService` 拆分

目标拆分：

| 目标服务 | 职责 |
|---|---|
| `DictApplicationService` | 字典管理 |
| `ConfigApplicationService` | 系统参数管理 |
| `NoticeApplicationService` | 公告管理 |
| `CategoryRuleApplicationService` | 分类规则管理 |
...
目标：把当前完整 SQL 脚本升级为可持续演进的数据库迁移体系。

执行项：

| 序号 | 事项 | 范围 | 验收标准 |
|---|---|---|---|
| 3.1 | 引入 Flyway | `pom.xml`、`resources/db/migration` | 新环境启动时可自动执行迁移 |
| 3.2 | 当前 SQL 转基线迁移 | `enterprise_auth_platform.sql` | 当前结构和基础数据能作为 V1 初始化 |
| 3.3 | 建立迁移命名规范 | `Vxxx__description.sql` | 后续结构、索引、种子数据都走版本化脚本 |
| 3.4 | 核验索引 | 用户、审计、租户、角色资源表 | 只根据实际查询路径补充索引，不盲目照旧计划添加 |
| 3.5 | 明确全局表与租户表 | MyBatis 租户拦截配置 | 全局表不被错误追加租户条件 |
...
| 序号 | 事项 | 验收标准 |
|---|---|---|
| 4.1 | 建立 `ModuleRegistry` | 路由、菜单、权限从模块清单汇总 |
| 4.2 | 迁移现有路由定义 | 不再维护多处 routeKey/path/icon 映射 |
| 4.3 | 实现真正动态路由注册 | `registerDynamicRoutes` 不再是空操作 |
| 4.4 | 拆分认证状态 | 认证态、会话态、租户态、权限快照职责分离 |
| 4.5 | 增强 HTTP 请求治理 | 幂等请求可重试，写操作默认不重试，支持请求取消和统一错误码 |
| 4.6 | 抽出通用 CRUD 组合能力 | 列表、搜索、表单弹窗、详情抽屉不再每个页面重复写 |
...
第一轮只做“基座骨架和关键拆分”，不一次性重写所有业务。

```mermaid
flowchart TD
    A[修订优化计划与基线测试] --> B[建立后端 common/modules/infrastructure 边界]
    B --> C[拆 AuthService / TenantManagementService / SystemManagementService]
    C --> D[引入 Flyway 基线迁移]
    D --> E[建立前端 ModuleRegistry]
    E --> F[拆分前端认证状态与 HTTP 治理]
    F --> G[补齐基座扩展文档]
```

| 顺序 | 任务 | 状态目标 |
|---|---|---|
| 1 | 重写优化计划清单（v2） | 模块边界、目录归属、协作规则达成口径一致 |
| 2 | 盘点现有测试并锁定回归链路 | 阶段 0 锁定的链路全部有可执行测试 |
| 3 | 建立后端八模块目录骨架（含 `dept`） | 新旧代码可并存迁移 |
| 4 | 拆解 `security/` 包按三分法归位 | `common/security`、`modules/auth`、`infrastructure/security` 各归其位 |
| 5 | 拆分 `AuthService` | 登录、注册、会话、租户切换、权限快照独立 |
| 6 | 拆分 `TenantManagementService` | 租户生命周期、能力、日志独立 |
| 7 | 拆分 `SystemManagementService` | 字典、参数、公告、分类独立 |
| 8 | 引入 Flyway 基线迁移 | 数据库初始化进入版本化迁移 |
| 9 | 建立前端 `app/registry/` 模块注册表 | 路由、菜单、权限统一声明 |
| 10 | 补齐扩展文档 | 新模块、新项目有明确接入方式 |
...
| 风险 | 影响 | 缓解措施 |
|---|---|---|
| 大规模移动包导致回归 | 高 | 先建立基线测试，分模块迁移，每轮保持接口兼容 |
| `common` / `infrastructure/config` 再次变成杂物间 | 高 | 通过模块边界规范限制；在 PR Review Checklist 中强制核对归属表 |
| 模块间私下耦合（绕过 application 接口） | 高 | 引入 ArchUnit 规则在 CI 阶段校验；新增 Mapper 注入时优先 Review |
| 阶段大改无法回滚 | 高 | **每阶段独立 PR + Git 标签（`phase-1`/`phase-2.1` 等），保留可回退点；阶段间不交叉提交** |
| Flyway 基线迁移与现有库冲突 | 中 | 未上线项目优先以干净库验证；本地库走 `baseline-on-migrate=true` 流程，禁止改 V1 |
| 串行拆分周期被压缩 | 中 | 优先级清晰：`Auth` 不通过不进 `Tenant`；不要并行抢工时 |
| 缓存引入脏数据 | 中 | 先定义失效策略，再扩大缓存范围 |
| 前端动态路由改造影响权限 | 中 | 先迁移菜单和权限映射，再启用真实动态注册；保留旧路由作为 fallback 一个迭代 |
...
| 阶段 | 验收标准 |
|---|---|
| 阶段 0 | 文档准确，核心链路有可执行回归验证 |
| 阶段 1 | 后端目录形成 `common / modules / infrastructure` 边界 |
| 阶段 2 | 三个大 Service 完成拆分，外部接口行为不变 |
| 阶段 3 | Flyway 可完成干净库初始化 |
| 阶段 4 | 前端路由、菜单、权限由模块清单统一注册 |
